...
class GameSessionAccessor(BaseAccessor):
    ...
    async def add_questions_to_session(
        self, session_id: int, theme_limit: int, questions_points: list[int]
                ...
                for theme in themes:
                    ...
                    for req_points in questions_points:
                        questions_list = await self.app.store.quizzes.list_questions(
                            theme_id=theme.id, points=req_points
                        )
    ...
    async def set_answerer(
        self, session_id: int, to_set: Union[list[int], int]
    ) -> Player:
        if type(to_set) == list:
    ...
    async def add_points_to_player(
        self, session_id: int, player_id: int, points: int
    ) -> int:
        async with self.app.database.session() as session:
            async with session.begin():
