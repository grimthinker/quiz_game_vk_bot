...
class BotManager:
    ...
    async def on_start(self, chat_id: int, player_id: int) -> None:
        ...
        await self.send_message(peer_id=chat_id, message=MessageHelper.started(name=player.name))
    ...
    async def on_participate(self, chat_id: int, player_id: int) -> None:
        ...
        if player_id in session_players:
            await self.send_message(peer_id=chat_id, message=MessageHelper.player_already_added(name=player.name))
            return
        await self.app.store.game_sessions.add_player_to_session(player_id, session_id)
        await self.send_message(peer_id=chat_id, message=MessageHelper.new_player_added(name=player.name))
    ...
    async def on_run(self, chat_id: int, player_id: int) -> None:
        ...
        if session.creator == player_id:
            ...
            await self.send_message(peer_id=chat_id,
                                    message=MessageHelper.not_creator_to_run(name=player.name,
        ...
        if len(session_players) > MAX_PLAYERS:
            ...
            return
        questions = await self.app.store.game_sessions.add_questions_to_session(session.id,
    ...
    async def on_choosing_answer(self, chat_id: int, is_correct: bool, player_id: int) -> None:
        ...
        if player_id not in players:
            await self.send_message(peer_id=chat_id, message=MessageHelper.can_not_answer(name=player.name))
            return
    ...
    async def on_correct_answer(self, chat_id: int, session_id: int, player_id: int, question: Question,
        ...
        await self.send_message(peer_id=chat_id, message=MessageHelper.answered_correct(name=player.name,
    ...
    async def on_wrong_answer(self, chat_id: int, session_id: int, player_id: int, question: Question, question_id: int
        ...
        await self.send_message(peer_id=chat_id, message=MessageHelper.answered_wrong(name=player.name,
                                                                                      points=str(-question.points),
                                                                                      curpoints=str(current_points)))
    ...
    async def after_answering(self, chat_id: int, session_id: int, player_id: int) -> None:
        ...
        if has_unanswered:
            ...
            await self.send_message(peer_id=chat_id, message=MessageHelper.choose_question(name=player.name),
                                    keyboard=KeyboardHelper.generate_questions_keyboard(questions=questions))
        else:
    ...
    async def on_stop(self, chat_id: int, player_id: int) -> None:
        ...
        if session.creator != player_id:
            await self.send_message(peer_id=chat_id,
                                    message=MessageHelper.not_creator_to_stop(name=player.name,
                                                                              creator_name=creator.name))
            return
        results = await self.end_game_session(session_id)
        await self.send_message(peer_id=chat_id,
    ...
    async def get_questions_of_session(self, session_id: int, answered: Optional[bool] = None, id_only: bool = False,
                                       to_dict: bool = True) -> Union[list[Question], dict[str, dict[int, Question]], None]:
