...
from app.game_session.models import GameSession, Player, SessionState, StatesEnum
...
class BotManager:
    ...
    async def on_bot_start(self, *_: list, **__: dict) -> None:
        ...
        chats = await self.app.store.game_sessions.list_chats(req_cnd=StatesEnum.SESSION_NEEDED)
        for chat_id in chats:
            await self.send_message(peer_id=chat_id,
                                    message=MessageHelper.initial,
                                    keyboard=KeyboardHelper.generate_initial_keyboard())

        chats = await self.app.store.game_sessions.list_chats(req_cnd=StatesEnum.PREPARING)
        for chat_id in chats:
            await self.send_message(peer_id=chat_id,
                                    message=MessageHelper.preparing,
                                    keyboard=KeyboardHelper.generate_preparing_keyboard())

        chats = await self.app.store.game_sessions.list_chats(req_cnd=StatesEnum.WAITING_QUESTION)
        for chat_id in chats:
            chat_sessions = await self.app.store.game_sessions.list_sessions(id_only=True,
                                                                             chat_id=chat_id,
        ...
        chats = await self.app.store.game_sessions.list_chats(req_cnd=StatesEnum.WAITING_ANSWER)
        for chat_id in chats:
            chat_sessions = await self.app.store.game_sessions.list_sessions(id_only=True,
                                                                             chat_id=chat_id,
    ...
    async def on_chat_inviting(self, chat_id: int) -> None:
        ...
        await self.send_message(peer_id=chat_id, message=MessageHelper.bot_added_to_chat)
        await self.send_message(peer_id=chat_id, message=MessageHelper.initial,
                                keyboard=KeyboardHelper.generate_initial_keyboard())

    async def on_start(self, chat_id: int, player_id: int) -> None:
        chat_running_sessions = await self.app.store.game_sessions.list_sessions(id_only=True,
                                                                                 chat_id=chat_id,
                                                                                 creator_id=player_id,
                                                                                 req_cnds=[StatesEnum.PREPARING,
        ...
        await self.send_message(peer_id=chat_id, message=MessageHelper.started(name=player_id))
        await self.send_message(peer_id=chat_id, message=MessageHelper.preparing,
                                keyboard=KeyboardHelper.generate_preparing_keyboard())
    ...
    async def on_participate(self, chat_id: int, player_id: int) -> None:
        ...
        if chat_sessions:
            session_id = chat_sessions[0]
            session_players = await self.app.store.game_sessions.list_players(id_only=True, session_id=session_id)
        else:
            await self.send_message(peer_id=chat_id, message=MessageHelper.no_preparing_session)
            return
        if player_id in session_players:
            await self.send_message(peer_id=chat_id, message=MessageHelper.player_already_added(name=player_id))
            return
        await self.app.store.game_sessions.add_player_to_session(player_id, session_id)
        await self.send_message(peer_id=chat_id, message=MessageHelper.new_player_added(name=player_id))
    ...
    async def on_run(self, chat_id: int, player_id: int) -> None:
        ...
        if session.creator == player_id:
            ...
            await self.send_message(peer_id=chat_id,
                                    message=MessageHelper.not_creator_to_run(name=player_id,
                                                                             creator_name=session.creator))
            return
        if len(session_players) < MIN_PLAYERS:
            await self.send_message(peer_id=chat_id, message=MessageHelper.not_enough_players)
            return
        if len(session_players) > MAX_PLAYERS:
            await self.send_message(peer_id=chat_id, message=MessageHelper.too_many_players)
            return
        questions = await self.add_questions_to_session(session.id)
        answerer = await self.app.store.game_sessions.set_answerer(session_id, to_set=session_players)
        await self.app.store.game_sessions.set_session_state(session.id, StatesEnum.WAITING_QUESTION)
        await self.send_message(peer_id=chat_id, message=MessageHelper.start_quiz)
        await self.send_message(peer_id=chat_id,
                                message=MessageHelper.choose_question(name=answerer.id),
                                keyboard=KeyboardHelper.generate_questions_keyboard(questions=questions))
    ...
    async def on_choosing_question(self, chat_id: int, question_id: int, player_id: int) -> None:
        ...
        if is_answered:
            await self.send_message(peer_id=chat_id, message=MessageHelper.question_already_answered)
            return
        question = await self.app.store.game_sessions.set_current_question(session_id, question_id)
        await self.app.store.game_sessions.set_session_state(session_id, StatesEnum.WAITING_ANSWER)
        await self.send_message(peer_id=chat_id,
                                message=MessageHelper.question(question=question.title),
                                keyboard=KeyboardHelper.generate_answers_keyboard(question=question))

    async def on_choosing_answer(self, chat_id: int, is_correct: bool, player_id: int) -> None:
        chat_sessions = await self.app.store.game_sessions.list_sessions(id_only=True,
                                                                         chat_id=chat_id,
                                                                         req_cnds=[StatesEnum.WAITING_ANSWER])
        if chat_sessions:
            ...
            await self.send_message(peer_id=chat_id, message=MessageHelper.no_running_session)
            return
        players = await self.app.store.game_sessions.list_players(id_only=True, session_id=session_id, can_answer=True)
        if player_id not in players:
            await self.send_message(peer_id=chat_id, message=MessageHelper.can_not_answer(name=player_id))
            return
        await self.question_answered(chat_id, player_id, session_id, is_correct)

    async def question_answered(self, chat_id: int, player_id: int, session_id: int, is_correct: bool) -> None:
        session_state = await self.get_session_by_id(session_id, return_state=True)
    ...
    async def on_correct_answer(self, chat_id: int, session_id: int, player_id: int, question: Question,
        ...
        current_points = await self.app.store.game_sessions.add_points_to_player(session_id, player_id, question.points)
        await self.app.store.game_sessions.restore_answering(session_id)
        await self.send_message(peer_id=chat_id, message=MessageHelper.answered_correct(name=player_id,
    ...
    async def on_wrong_answer(self, chat_id: int, session_id: int, player_id: int, question: Question, question_id: int
        ...
        await self.send_message(peer_id=chat_id, message=MessageHelper.answered_wrong(name=player_id,
                                                                                      points=str(-question.points),
                                                                                      curpoints=str(current_points)))
        await self.app.store.game_sessions.forbid_answering(session_id, player_id)
        no_players_left = await self.app.store.game_sessions.check_if_no_players_left(session_id)
        if no_players_left:
            await self.send_message(peer_id=chat_id, message=MessageHelper.no_players_left(answer=answer.title))
            await self.app.store.game_sessions.set_question_answered(question_id)
            await self.after_answering(chat_id, session_id, player_id)
        else:
    ...
    async def after_answering(self, chat_id: int, session_id: int, player_id: int) -> None:
        ...
        if has_unanswered:
            questions = await self.get_questions_of_session(session_id, answered=False)
            await self.app.store.game_sessions.restore_answering(session_id)
            await self.app.store.game_sessions.set_session_state(session_id, StatesEnum.WAITING_QUESTION)
            await self.send_message(peer_id=chat_id, message=MessageHelper.choose_question(name=player_id),
                                    keyboard=KeyboardHelper.generate_questions_keyboard(questions=questions))
        else:
            results = await self.end_game_session(session_id)
            await self.send_message(peer_id=chat_id, message=MessageHelper.quiz_ended(results=results),
                                    keyboard=KeyboardHelper.generate_initial_keyboard())
    ...
    async def on_stop(self, chat_id: int, player_id: int) -> None:
        ...
        if chat_sessions:
            ...
            await self.send_message(peer_id=chat_id, message=MessageHelper.no_session_to_stop)
            return
        if session.creator != player_id:
            await self.send_message(peer_id=chat_id,
                                    message=MessageHelper.not_creator_to_stop(name=player_id,
                                                                              creator_name=session.creator))
            return
        results = await self.end_game_session(session_id)
        await self.send_message(peer_id=chat_id,
                                message=MessageHelper.quiz_ended_on_stop(name=player_id, results=results),
                                keyboard=KeyboardHelper.generate_initial_keyboard())
    ...
    async def on_show_results(self, chat_id: int):
        ...
        if not last_session_id:
            ...
            return
        results = await self.app.store.game_sessions.get_session_results(last_session_id[0])
        await self.send_message(peer_id=chat_id, message=MessageHelper.just_show_results(results=results))

    async def last_session(self, chat_id: int) -> list[int]:
        game_session_id = await self.app.store.game_sessions.list_sessions(id_only=True,
                                                                           chat_id=chat_id,
                                                                           last=True)
    ...
    async def end_game_session(self, session_id) -> dict[int, int]:
        await self.app.store.game_sessions.set_session_state(session_id, StatesEnum.ENDED)
        results = await self.app.store.game_sessions.get_session_results(session_id)
        return results

    async def send_message(self, peer_id: int, message: str, keyboard: Optional[str] = None) -> None:
